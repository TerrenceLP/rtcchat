/**
 * Contains the Room connection configuration.
 ...
 * @method _fetchSession
 * @private
 * @since 0.6.8
 * @for Skylink
 */
Skylink.prototype._fetchSession = function () {
  var _this = this;
  var xhr = null;


  // Use XDomainRequest
  if (_this._session.useXDomainRequest) {
    xhr = new XDomainRequest();
    xhr.setContentType = function (contentType) {
      xhr.contentType = contentType;
    };

    log.log([null, 'Room', _this._room.selected, 'Using XDomainRequest to fetch Room session']);
  ...
  // Use XMLHttpRequest
    ...
    log.log([null, 'Room', _this._room.selected, 'Using XMLHttpRequest to fetch Room session']);
  }
  ...
  xhr.onload = function () {
    ...
    log.debug([null, 'Room', _this._room.selected, 'Received response from API server ->'], response);

    if (!response.success) {
      log.debug([null, 'Room', _this._room.selected, 'Failed initialising session ->'], response.message);
    }

    log.debug([null, 'Room', _this._room.selected, 'Session has been initialized']);
    _this._readyState = _this.READY_STATE_CHANGE.COMPLETED;
    _this._trigger('readyStateChange', _this.READY_STATE.COMPLETED, null, _this._room.selected);
  };

  // XMLHttpRequest.onerror event
  xhr.onerror = function (error) {
    log.error([null, 'Room', _this._room.selected, 'Failed retrieving response from API server ->'], error);

    _this._readyState = _this.READY_STATE_CHANGE.ERROR;
    _this._trigger('readyStateChange', _this.READY_STATE_CHANGE.ERROR, {
    ...
    }, _this._room.selected);
  };

  // XMLHttpRequest.onprogress
  xhr.onprogress = function () {
    log.debug([null, 'Room', _this._room.selected, 'Retrieving response from API server']);
  };

  xhr.open('GET', _this._session.path, true);
  xhr.send();
};
...
/**
 * Constructs the Room session path for fetching.
 * @method _constructSessionPath
 * @private
 * @since 0.6.8
 * @for Skylink
 */
Skylink.prototype._constructSessionPath = function () {
  var _this = this;
  var append = '?&';

  // Construct the path
  _this._session.path = _this._session.protocol + '//' + _this._room.server + '/api/' + _this._room.appKey + '/' +
    _this._room.selected;

  // Check if credentials is needed
  if (_this._room.credentials) {
    _this._session.path += _this._room.credentials.startDateTime + '/' + _this._room.credentials.duration +
      append + 'cred=' + _this._room.credentials.credentials;
    append = '&';
  }

  // Check if region is needed
  if (_this._room.region) {
    _this._session.path += append + 'rg=' + _this._room.region;
    append = '&';
  }

  // Add the random string
  _this._session.random = (new Date()).getTime();
  _this._session.path += append + 'rand=' + _this._session.random;

  // Check if XDomainRequest is used
  _this._session.useXDomainRequest = typeof window.XDomainRequest === 'function' ||
 ...
 * @for Room
 */
Skylink.prototype.Room.prototype.name = null;

/**
 * Contains the Room

/**
 * Contains the default configuration for init().
 * @attribute
